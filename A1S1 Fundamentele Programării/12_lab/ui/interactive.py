...
class UIInteractive:
	def __init__(self, student_service, discipline_service, grade_service):
		...
		self.__commands = {
			...
			"add random students": self.__add_random_students,
			...
			"print grades sorted by name and value": self.__print_grades_sorted_by_name_and_value,
			"print top 20 percent": self.__print_top_20_percent,
			"print top 3 students": self.__print_top_3_students,
		}
	...
	def __optional_input(self, name, converter, default):
		value = input(f"{name} = ")
		if value == "":
			return default
		else:
	...
	def __print_sorted_grades(self):
		...
		student_grade_dtos = self.__grade_service.get_grades_with_discipline_sorted_by_criterion(discipline_id, criterion)
		
		print(f"{'Name':15} {'Value':4}")
		for student_grade_dto in student_grade_dtos:
			student_name = student_grade_dto.get_student_name()
			value = student_grade_dto.get_value()
			
			print(f"{student_name:15} {value:4}")
	
	def __print_grades_sorted_by_name_and_value(self):
		student_grade_dtos = self.__grade_service.get_grades_sorted_by_student_name_and_value()
	...
	def __print_top_20_percent(self):
		top_20_percent = self.__grade_service.get_top_20_percent_of_students()
		
		print(f"{'Name':15} {'Average':4}")
		for student_grade_average_dto in top_20_percent:
			student_name = student_grade_average_dto.get_student_name()
			average = student_grade_average_dto.get_average()
			
			print(f"{student_name:15} {average:4}")
	
	def __print_top_3_students(self):
		top_3_students = self.__grade_service.get_top_3_students_by_average()
		
		print(f"{'Name':15} {'Average':4}")
		for student_grade_average_dto in top_3_students:
			student_name = student_grade_average_dto.get_student_name()
			average = student_grade_average_dto.get_average()
