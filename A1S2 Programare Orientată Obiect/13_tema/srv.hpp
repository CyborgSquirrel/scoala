...
public:
	...
	std::vector<Carte> getCarti() const;
	
	// Exporta cartile din cos in fisierul dat prin file_path.
	// in: file_path -- path-ul fisierul in care vor fi salvate cartile
	void csvExport(const std::string &file_path) const;

// addedInchiriere
private:
	size_t m_lastAddedInchiriereSignalId;
	std::map<size_t, std::function<void ()>> m_addedInchiriereSignals;
	void addedInchiriere();
public:
	size_t connectAddedInchiriere(std::function<void ()> signal);
	void disconnectAddedInchiriere(size_t id);

// emptiedInchirieri
private:
	size_t m_lastEmptiedInchirieriSignalId;
	std::map<size_t, std::function<void ()>> m_emptiedInchirieriSignals;
	void emptiedInchirieri();
public:
	size_t connectEmptiedInchirieri(std::function<void ()> signal);
	void disconnectEmptiedInchirieri(size_t id);
};
