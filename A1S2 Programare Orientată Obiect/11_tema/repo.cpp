...
void RepoCarti::add(const Carte &carte) {
	...
	if (index != (size_t) -1) { throw RepoCartiException("cartea furnizata este deja in repo"); }
	this->carti.push_back(carte);
}

void RepoCarti::erase(size_t id) {
	size_t index = this->get_id_index(id);
	if (index == (size_t) -1) { throw RepoCartiException("nu exista carte cu id-ul furnizat in repo"); }
...
void RepoInchirieriCarte::add(const InchiriereCarte &inchiriere_carte) {
	...
	if (index != (size_t) -1) { throw RepoInchirieriCarteException("inchirierea furnizata este deja in repo"); }
	this->inchirieri_carte.push_back(inchiriere_carte);
}

void RepoInchirieriCarte::erase(size_t id) {
	size_t index = this->get_id_index(id);
	if (index == (size_t) -1) { throw RepoInchirieriCarteException("nu exista inchiriere cu id-ul furnizat in repo"); }
...
void test_repo_inchirieri_carte() {
	...
	size_t a_id = 0; InchiriereCarte a {a_id};
	size_t b_id = 1; InchiriereCarte b {b_id};
	size_t c_id = 2; InchiriereCarte c {c_id};
