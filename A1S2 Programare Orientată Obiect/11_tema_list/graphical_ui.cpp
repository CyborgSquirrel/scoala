...
QVariant SrvInchirieriCarteModel::data(const QModelIndex &index, int role) const {
	...
	if (role == Qt::DisplayRole || role == Qt::EditRole) {
		int column = index.column();
		if (column == 0) return QVariant(QString::fromStdString(carte.get_titlu()));
		else return QVariant();
	} else {
		return QVariant();
	}
...
GraphicalUi::GraphicalUi(
	...
	// PAGE CART
		...
		auto list_srv_inchirieri_carte = new QListView;
		SrvInchirieriCarteModel *model_srv_inchirieri_carte = new SrvInchirieriCarteModel(this->srv_inchirieri_carte);
		list_srv_inchirieri_carte->setModel(model_srv_inchirieri_carte);
		layout_cart->addWidget(list_srv_inchirieri_carte);
