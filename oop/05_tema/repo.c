...
int __repo_cheltuieli_find(
	...
	for (int i = 0; i < repo_cheltuieli->vec.len; ++i) {
		...
		assert(vec_get(&repo_cheltuieli->vec, (void *) &cheltuiala, i) == NULL);
		if (cheltuiala.id == id) {
			return i;
		}
	}
...
const char *const repo_cheltuieli_add(
	...
	vec_push(&repo_cheltuieli->vec, (void *) &cheltuiala);
	return NULL;
}
...
const char *const repo_cheltuieli_update(
	...
	int index = __repo_cheltuieli_find(repo_cheltuieli, cheltuiala.id);
	if (index == -1) return ERR_CHELTUIALA_NOT_IN_REPO;
	assert(vec_set(&repo_cheltuieli->vec, index, (void *) &cheltuiala) == NULL);
	return NULL;
}
...
struct Vec repo_cheltuieli_get_all(
	...
	assert(repo_cheltuieli);
	struct Vec clone;
	vec_clone(&clone, &repo_cheltuieli->vec);
	return clone;
}
...
void test_repo_cheltuieli_get_all() {
	struct RepoCheltuieli repo_cheltuieli = repo_cheltuieli_new();
	struct Cheltuiala cheltuiala_1 = cheltuiala_new(0, data_new(22, 1, 2014), 3.50, MANCARE);
	...
	struct Vec vec = repo_cheltuieli_get_all(&repo_cheltuieli);
	for (int i = 0; i < 100; ++i) {
		struct Cheltuiala cheltuiala_get;
		vec_get(&vec, &cheltuiala_get, i);
		assert(cheltuiala_eq(&cheltuiala_1, &cheltuiala_get));
	}
	vec_drop(&vec);
