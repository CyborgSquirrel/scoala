...
JAVA_PATHS = (JAVA_PATH / f"{i}.java" for i in itertools.count())

SQL_PATH = BIN_PATH / "sql"
SQL_PATHS = (SQL_PATH / f"{i}.sql" for i in itertools.count())

RE_CODE_FENCE = re.compile(
    "\\s*```"  # start marker
    "[^\\n]*\\n"  # language
    "("  # source code
        # makes sure "```" does not show up in the source
        "(?:(?:(?!```).)*\\n)*"
    ")"
    "\\s*```"  # end marker
)
...
def compile_bash(name: str, source: str, args: typing.Dict):
    target_path = TARGET_PATH / name
    target_path.write_text(
        "#!/usr/bin/env bash\n"
        + source
...
def compile_c(name: str, source: str, args: typing.Dict):
    target_path = TARGET_PATH / name
    with tempfile.TemporaryDirectory() as tmp_dir:
        tmp_dir = pathlib.Path(tmp_dir)
        source_path = tmp_dir / "source.c"
...
def compile_sql(name: str, source: str, args: typing.Dict):
    sql_path = next(SQL_PATHS)
    sql_path.write_text(source)
    
    target_path = TARGET_PATH / name
    target_path.write_text(
        "#!/usr/bin/env sh\n"
        f"sqlite3 ':memory:' < '{sql_path}'\n",
    )
    target_path.chmod(
          stat.S_IRUSR
        | stat.S_IWUSR
