...
class GradeService:
	...
	def get_grades_with_discipline_sorted_by_criterion(self, discipline_id, criterion):
		...
		mysort.selection_sort(student_grade_dtos, key=key, reverse=reverse)
	...
	def get_grades_sorted_by_student_name_and_value(self):
		''' Gets list of all the grades sorted first by the student's name, and then by the grade's value.
		output: list of StudentGradeDTO
		'''
		student_grade_dtos = self.__grade_repo.get_student_grade_dtos()
		...
		def key(a):
			return (a.get_student_name(), a.get_value())
		
		mysort.selection_sort(student_grade_dtos, key=key)
	...
	def get_top_20_percent_of_students(self):
		...
		mysort.shake_sort(student_grade_average_dtos, key=key, reverse=True)
	...
	def erase_student_and_all_their_grades(self, student_id):
		...
		for grade in self.__grade_repo.get_grades():
			...
			if grade.get_student_id() == student_id:
				self.erase_grade(grade_id)
				self.erase_student_and_all_their_grades(student_id)
				return
		self.__student_repo.erase(student_id)
	...
	def erase_discipline_and_all_its_grades(self, discipline_id):
		...
		for grade in self.__grade_repo.get_grades():
			...
			if grade.get_discipline_id() == discipline_id:
				self.erase_grade(grade_id)
				self.erase_discipline_and_all_its_grades(discipline_id)
				return
		self.__discipline_repo.erase(discipline_id)
