...
class GradeService:
	...
	def get_grades_with_discipline_sorted_by_criterion(self, discipline_id, criterion):
		...
		grades = [grade for grade in grades if grade.get_discipline_id() == discipline_id]
		reverse = False
		if criterion == "name":
			def key(a):
	...
	def get_top_20_percent_of_students(self):
		...
		for student in students:
			...
			if student_id in student_grade_sum and student_id in student_grade_amount:
				...
				students_and_averages.append((student_id, grade_average))
		
		def key(a):
			return a[1]
		students_and_averages.sort(key=key, reverse=True)
		
		cutoff = int(len(students_and_averages) * 0.20)
		return students_and_averages[:cutoff]
