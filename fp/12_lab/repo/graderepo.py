...
class InMemoryGradeRepo:
	...
	def get_student_grade_dtos(self):
		''' Returns a list of all the grades, as StudentGradeDTOs.
	...
	def get_student_grades_by_discipline_id(self, discipline_id):
		''' Returns a list of all the grades, as StudentGradeDTOs, with specified discipline_id.
...
class FileGradeRepo(InMemoryGradeRepo):
		...
		try:
			with open(self.__filename, "r") as f:
				for line in f:
					...
					if line.strip() != "":
						line = line.split(";")
						grade = Grade(int(line[0]), int(line[1]), int(line[2]), float(line[3]))
						super().store(grade)
	...
	def __store_all_to_file(self):
		with open(self.__filename, "w") as f:
			...
			for grade in grades:
				print(f"{grade.get_grade_id()};{grade.get_student_id()};{grade.get_discipline_id()};{grade.get_value()}", file=f)
